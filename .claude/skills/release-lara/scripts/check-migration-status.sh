#!/usr/bin/env bash
# Verify LARA's schema state from the database's own record of applied migrations.
#
# This reads `rake db:migrate:status`, whose answer comes from the `schema_migrations`
# table. That table is the primary source: it is what Rails itself consults to decide
# which migrations are still pending. Prefer it over verify-migration.sh whenever the
# question is "is the schema where it should be", because:
#
...
# 20251209102615. This differs from verify-migration.sh on purpose, and the difference is
...
#
# Log groups are not named after the stacks: staging is "authoring-lara-staging" but
# production is "lara-production-ecs" (not "lara-ecs-production", which is the stack).
# Streams are "lara/App/<task-id>" in both.
set -uo pipefail

if [ "$#" -lt 3 ]; then
  echo "usage: $(basename "$0") <log-group> <task-id> <applied|report> [expected-version ...]" >&2
  echo "  e.g. $(basename "$0") authoring-lara-staging abc123 applied 20251209102615" >&2
  exit 2
fi

GROUP="$1"; TASK_ID="$2"; MODE="$3"; shift 3
EXPECTED=("$@")
STREAM="lara/App/${TASK_ID}"
...
for want in ${EXPECTED+"${EXPECTED[@]}"}; do
  if ! [[ "$want" =~ ^[0-9]+$ ]]; then
    ...
    echo "  pass versions (20251209102615), not class names (AddFooToBars)" >&2
    exit 2
  fi
done
...
# stops growing across two consecutive reads instead. LARA prints hundreds of rows and
# the stream fills incrementally, so a single read can easily catch a partial table and
# report healthy migrations as missing.
...
if [ -z "$ROWS" ]; then
  ...
  if [ -z "$LOG" ]; then
    ...
    echo "  the stream has output but no status table; check the command override was"
    echo "  'rake db:migrate:status' and that the entryPoint runs it under bundle exec"
  fi
  exit 1
fi
...
if [ "$n_down" -gt 0 ]; then
  ...
  # pending, and several hundred lines would bury the summary above.
  head -20 <<<"$DOWN"
