#!/usr/bin/env bash
# Verify a LARA migration task actually applied its migrations, by reading the
...
#
# Log groups are not named after the stacks: staging is "authoring-lara-staging"
# but production is "lara-production-ecs" (not "lara-ecs-production", which is the
# stack). Streams are "lara/App/<task-id>" in both.
#
...
if [ "$#" -lt 2 ]; then
  ...
  echo "  e.g. $(basename "$0") authoring-lara-staging abc123 AddFooToBars" >&2
  exit 2
fi
...
while [ $SECONDS -lt $DEADLINE ]; do
  ...
  # yet will never appear, and a wrong region or a transposed group name presents as
  # exactly that.
