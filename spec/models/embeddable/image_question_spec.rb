...
describe Embeddable::ImageQuestion do
  ...
  describe '#to_hash' do
    it 'has interesting attributes' do
      expected = { name: image_question.name, prompt: image_question.prompt, bg_source: image_question.bg_source, bg_url: image_question.bg_url }
  ...
  describe '#duplicate' do
    it 'returns a new instance with copied attributes' do
      image_question.duplicate.should be_a_new(Embeddable::ImageQuestion).with( name: image_question.name, prompt: image_question.prompt, bg_source: image_question.bg_source, bg_url: image_question.bg_url )
    end
  end
