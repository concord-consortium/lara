...
describe LightweightActivitiesController do
  ...
  context 'when the current user is an author' do
    ...
    before(:each) do
      make_collection_with_rand_modication_time(:activity,3, :is_official => false)
      make_collection_with_rand_modication_time(:activity,4, :is_official => true)
    end

    describe '#index' do
      it 'has a list of public and owned activities' do
        # User is an admin, so all public activities will be shown
        get :index, 'filter' => {'my' => '1' }
