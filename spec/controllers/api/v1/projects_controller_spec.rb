...
describe Api::V1::ProjectsController do
  ...
  describe "#show" do
    it "returns a JSON string for a project with a specific ID" do
      xhr :get, "show", params: {id: project1.id}
      expect(response.status).to eq(200)
      expect(response.content_type).to eq("application/json")
    ...
    it "returns a 404 error for a non-existent project ID" do
      xhr :get, "show", params: {id: 123456789}
  ...
  describe "#create" do
    it "returns a success message and values for the newly created project" do
      xhr :post, "create", params: {project: {title: "My New Project", project_key: "my-new-project"}}
  ...
  describe "#update" do
    it "returns a success message and a JSON string when a project is updated" do
      ...
      xhr :post, "update", params: {id: project1.id, project: {title: "New Project Title", admins: admins}}
      expect(response.status).to eq(200)
      expect(response.content_type).to eq("application/json")
      response_body = JSON.parse(response.body, symbolize_names: true)
      project = response_body[:project]
    ...
    it "allows removing project admins" do
      ...
      xhr :post, "update", params: {id: project1.id, project: {title: "New Project Title", admins: []}}
      expect(response.status).to eq(200)
      expect(response.content_type).to eq("application/json")
      response_body = JSON.parse(response.body, symbolize_names: true)
      project = response_body[:project]
  ...
  describe "#destroy" do
    it "returns a success message when a project is deleted" do
      xhr :post, "destroy", params: {id: project1.id}
      expect(response.status).to eq(200)
      expect(response.content_type).to eq("application/json")
      expect(response.body).to eq({
        success: true
